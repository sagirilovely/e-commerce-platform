...
export default {
  ...
  getOrdersOfMerchant:(req:Request,res:Response)=>{
  ...
  updateLogistics:(req:Request,res:Response)=>{
  ...
  updateRefund:(req:Request,res:Response)=>{
    const order_id=String(req.body.order_id);
    if(!order_id){
      res.status(500).json({
        message:"更新退款信息失败"
  ...
  delOrderByMerchant:(req:Request,res:Response)=>{
    const order_id=String(req.body.order_id);
    if(!order_id){
      res.status(500).json({
        message:"删除订单失败"
