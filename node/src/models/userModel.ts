...
export default {
    updateUserTrolley: async (userEmail: string,goods_id:string,goods_count:string):Promise<boolean|undefined> => {
        try {
            //通过邮箱查询到用户的购物车信息
            ...
            try{
                const [rows] = await promisePool.execute<ResultSetHeader>(`update purchasers
