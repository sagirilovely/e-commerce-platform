...
function sendEmail(to:string|undefined,subject:string,newVerifyCode:string,host:string):Promise<boolean>{
    return new Promise((resolve, reject)=>{
        nodemailer.createTransport(configMessage.eMailConfig).sendMail({
            ...
            <div>一定是有人用了你的邮箱咯,本小姐来都来了,你要不要..顺便来本小姐的网站看看?</div><br>
            <span>就..就是是这个哦:<a>http://${host}</a></span>
            <div>才..!才不是喜欢你!哼!!</div>
            `
